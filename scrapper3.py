...
def get_price(soup):
    ...
    prices = []
    price_tags = soup.find_all('div', class_='Flights-Results-FlightPriceSection right-alignment sleek')
...
for i in range(len(sources)):
    ...
    for j in tqdm.tqdm(range(num_days + 1)):
        ...
        try:
            ...
            show_more_button = driver.find_element(By.XPATH, '//a[@class = "moreButton"]')
            if show_more_button:
        ...
        if len(airlines) != len(prices) or len(airlines) != len(duration) or len(airlines) != len(total_stops):
    ...
    df = pd.DataFrame(data, columns=column_names)  # Convert list of dictionaries to dataframe
    df = df.replace('\n', '', regex=True)
    ...
    if not os.path.exists(file_name) or not os.path.isfile(file_name):
        try:
            ...
            print(f"Data for {sources[i]} => {destinations[i]} saved as {file_name}")
